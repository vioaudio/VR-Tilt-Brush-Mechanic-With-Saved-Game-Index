...
{
	auto Index = Cast<UPainterSaveGameIndex>(UGameplayStatics::LoadGameFromSlot(SLOT_NAME, 0)); //auto returns type in the function
	if (!Index)
	{
...
void UPainterSaveGameIndex::AddSlotName(ULightPainterSaveGame* NewSaveGame)
{
	SlotNames.Add(NewSaveGame->GetSlotName());
}
